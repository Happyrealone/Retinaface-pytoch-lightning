...
class DataGenerator(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img, target = self.get_random_data(img, target, self.img_size)
        img = np.array(np.transpose(img - np.array([104, 117, 123]), (2, 0, 1)), dtype=np.float32)
        ...
        for line in lines:
            ...
            if line.startswith('#'):
                if isFirst is True:
                    ...
                    labels.clear()
                path = line[2:]
                path = self.txt_path.replace(self.txt_path.split("/")[-1], 'images/') + path
